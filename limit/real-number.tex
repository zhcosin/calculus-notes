...
则$\forall t_a \in A, \forall t_b \in B$，有$t_a<t_b$，于是$A$有上界，$B$有下界，于是$A$有上确界$L$，$B$有下确界$U$，显然必有$L\leqslant U$，实际上必须$L=U$，不然的话，设$L<U$，由确界定义，分别存在有理数$r_1$和$s_1$，使得$r_1<x<s_1$，且$L-\frac{U-L}{10}<a^{r_1}<L<U<a^{s_1}<U+\frac{U-L}{10}$，从而
