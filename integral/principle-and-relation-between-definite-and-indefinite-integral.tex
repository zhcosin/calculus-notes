...
\begin{example}
  ...
  对于任意小的正实数 $\varepsilon>0$，我们需要找到另一个充分小的正实数$\delta>0$，使得对于满足小区间长度全部都小于$\delta$的任意一个分划，都有 $\sum_{i=1}^n\omega_i\Delta x_i<\varepsilon$.
  ...
  个，记这些自变量的集合为 $X$，即 $X=\{x|R(x) \geqslant \frac{1}{N}\}$.
  ...
  现在只要取 $N>\frac{2}{\varepsilon}$ 及 $\delta < \frac{\varepsilon}{2N^2}$，就有
