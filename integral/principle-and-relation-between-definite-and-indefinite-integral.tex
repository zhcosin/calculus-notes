...
定积分的定义中，小区间上的点 $\xi_i$ 的选取具备任意性，这为研究函数的可积性带来了难度，但是，无论 $\xi_i$ 如何选取，函数值$f(\xi_i)$ 必然只在所属小区间上函数值的上下确界所界定的范围内变动，由此引出 \emph{达布和}的概念.
...
从几何的角度，这意味着 \autoref{fig:integrable-condition} 中，阴影部分的面积在无限细分时能够成为无穷小. 换句话说，曲边梯形能够求面积，当且仅当它的边界面积为零.
\begin{figure}
  \centering
  \includegraphics[scale=0.7]{integral/pic/integrable-condition.pdf}
...
\begin{theorem}[基本积分表]
  \begin{enumerate}
    \item $ \int a \dif x = a x + C (a \in \mathrm{R})$
    \item $ \int x^a \dif x = \frac{x^{a+1}}{a+1} + C(a \neq -1)$
    \item $ \int \frac{1}{x} \dif x = \ln |x| + C $
    \item $ \int \sin x \dif x = -\cos x + C $
    \item $ \int \cos x \dif x = \sin x + C $
    \item $ \int \sec^2 x \dif x = \int \frac{1}{\cos^2 x} = \tan x + C $
    \item $ \int \csc^2 x \dif x = \int \frac{1}{\sin^2 x} = -\cot x + C $
    \item $ \int a^x \dif x = \frac{a^x}{\ln a} + C $ 特别的有 $ \int e^x \dif x = e^x + C $
    \item $ \int \frac{1}{1+x^2} \dif x = \arctan x + C $
    \item $ \int \frac{1}{\sqrt{1-x^2}} = \arcsin x + C $
    \item $ \int \cosh x = \sinh x + C $
    \item $ \int \sinh x = \cosh x + C $
  \end{enumerate}
\end{theorem}
通过对右边的原函数进行求导就可以验证以上积分法则.

根据函数求导的相关法则，可得不定积分的以下性质
\begin{theorem}[不定积分的性质]
  \begin{enumerate}
    \item $ \int (f(x)\pm g(x)) \dif x = \int f(x) \dif x \pm \int g(x) \dif x $
    \item $ \int a f(x) \dif x = a \int f(x) \dif x $
  \end{enumerate}
\end{theorem}
这表明不定积分运算是线性的，可以推广如下
\[ \int \sum_{m=1}^{n} a_m f_m(x) \dif x = \sum_{m=1}^{n} a_m \int f_m(x) \dif x \]
